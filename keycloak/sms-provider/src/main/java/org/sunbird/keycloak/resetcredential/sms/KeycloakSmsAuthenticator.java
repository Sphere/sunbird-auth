...
public class KeycloakSmsAuthenticator implements Authenticator {
  ...
  public void authenticate(AuthenticationFlowContext context) {
    ...
    logger.info("KeycloakSmsAuthenticator@authenticate - UserName = " + user.getUsername() + ", UserEmail = "
        + user.getEmail());
  ...
  private void navigateToEnterOTPPage(AuthenticationFlowContext context, Boolean flag) {
    if (flag) {
      ...
      Response challenge = context.form().setError("OTP could not be sent.").createForm("sms-validation-error.ftl");
  ...
  protected CODE_STATUS validateCode(AuthenticationFlowContext context) {
    ...
    List codeCreds = session.userCredentialManager().getStoredCredentialsByType(context.getRealm(), context.getUser(),
        KeycloakSmsAuthenticatorConstants.USR_CRED_MDL_SMS_CODE);
    /*
     * List timeCreds =
     * session.userCredentialManager().getStoredCredentialsByType(context.getRealm()
     * , context.getUser(),
    ...
    logger.debug("KeycloakSmsAuthenticator@validateCode " + "Expected code = " + expectedCode.getValue()
  ...
  public boolean configuredFor(KeycloakSession session, RealmModel realm, UserModel user) {
    logger.debug("KeycloakSmsAuthenticator@validateCode configuredFor called ... session=" + session + ", realm="
        + realm + ", user=" + user);
    return true;
  }
  ...
  public void setRequiredActions(KeycloakSession session, RealmModel realm, UserModel user) {
    logger.debug("KeycloakSmsAuthenticator@validateCode - setRequiredActions called ... session=" + session + ", realm="
        + realm + ", user=" + user);
  }
