...
public class KeycloakSmsAuthenticator implements Authenticator {
    ...
    public void authenticate(AuthenticationFlowContext context) {
        ...
        if (StringUtils.isNotBlank(mobileNumber) || StringUtils.isNotBlank(userEmail)) {
          ...
          String useSms = System.getenv(Constants.SEND_OTP_VIA_SMS);    // Aastrika specific
          if (StringUtils.isNotBlank(mobileNumber) && StringUtils.isNotBlank(useSms) && "true".equalsIgnoreCase(useSms)) {
            sendSMS(otpResponse, context, mobileNumber);
          }
          if (StringUtils.isNotBlank(userEmail)) {
              // Aastrika specific START here
